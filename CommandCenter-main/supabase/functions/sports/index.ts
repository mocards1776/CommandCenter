...
async function scrapeContract(name: string, hintUrl?: string | null, mlbId?: number | null) {
  ...
  return withBudget(HEAVY_MS, async () => {
    ...
    if (spotrac && hasContractBits(spotrac)) {
      // Spotrac-only wins still need BBRef service time / WAR when we scraped it.
      ...
      if (bb && "seasonWar" in bb && (bb as { seasonWar?: number | null }).seasonWar != null) {
        (spotrac as { seasonWar?: number | null }).seasonWar = (
          bb as { seasonWar?: number | null }
        ).seasonWar;
      }
      if (bb && "careerWar" in bb && (bb as { careerWar?: number | null }).careerWar != null) {
        (spotrac as { careerWar?: number | null }).careerWar = (
          bb as { careerWar?: number | null }
        ).careerWar;
      }
      return spotrac;
    }
