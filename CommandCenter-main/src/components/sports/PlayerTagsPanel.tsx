          ...
          {tagList.map((t) =>
            linkTags ? (
              <Link
                key={t.id}
                to={tagPath(t.tag)}
                className="inline-flex items-center rounded-md border border-sky-300/25 bg-sky-400/10 px-2.5 py-1 text-[11px] font-medium tracking-wide text-sky-100 transition hover:border-sky-300/45 hover:bg-sky-400/16"
              >
              ...
              <span
                key={t.id}
                className="inline-flex items-center rounded-md border border-sky-300/25 bg-sky-400/10 px-2.5 py-1 text-[11px] font-medium tracking-wide text-sky-100"
              >
                {displayPlayerTag(t.tag)}
              </span>
            ),
  ...
  return (
    <section className="bg-panel rounded-xl border border-white/[0.08] p-4">
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {isFavorite && (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-300/30 bg-amber-400/10 px-2.5 py-1 text-[12px] font-medium text-amber-100">
        ...
        {tagList.length === 0 && !isFavorite ? (
          ...
          tagList.map((t) => (
            <span
              key={t.id}
              className="inline-flex items-center gap-1.5 rounded-md border border-sky-300/25 bg-sky-400/10 px-2.5 py-1 text-[12px] font-medium text-sky-100"
            >
              {linkTags ? (
