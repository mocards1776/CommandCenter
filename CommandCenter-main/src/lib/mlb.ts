...
function buildHotSeat(
  ...
  if (marketPct != null && marketPct > 0) {
    // Kalshi is the heaviest single weight in the hot-seat score (by design).
    const marketPts = Math.round(marketPct * 0.95 * 10) / 10; // 50% → 47.5 heat
    factors.push({
      key: "market",
      label: "Kalshi %",
      points: marketPts,
      detail: `Kalshi ~${marketPct.toFixed(1)}% next-fired → +${marketPts.toFixed(1)} heat (heaviest factor)`,
    });
  }
